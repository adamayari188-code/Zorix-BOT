...
@bot.tree.command(name='rules', description='إرسال رسالة القوانين')
@app_commands.describe(channel='القناة المطلوب إرسال القوانين إليها')
async def send_rules_command(interaction: discord.Interaction, channel: discord.TextChannel):
    """إرسال رسالة القوانين إلى قناة محددة"""
    if not interaction.user.guild_permissions.manage_messages:
        await interaction.response.send_message('❌ ليس لديك صلاحيات إدارة الرسائل.', ephemeral=True)
        return
    
    rules_message = """**قوانين السيرفر**

مرحباً بك في مجتمعنا! نرجو منك قراءة القوانين والالتزام بها لضمان بيئة آمنة، محترمة، وممتعة للجميع.

1. **الاحترام أولاً**
   • يمنع الإساءة، الشتم، التنمر، أو التقليل من أي عضو.
   • احترم جميع الأعضاء والإدارة دون استثناء.

2. **منع السبام**
   • يمنع إرسال الرسائل المتكررة أو المنشن العشوائي.
   • يمنع استخدام البوتات أو أي وسيلة تسبب إزعاجاً للأعضاء.

3. **المحتوى المناسب**
   • يمنع نشر أي محتوى إباحي، عنصري، أو يحرض على الكراهية أو العنف.
   • يمنع إرسال الصور أو المقاطع الصادمة.

4. **الإعلانات**
   • يمنع الإعلان عن أي سيرفر، قناة، حساب، أو مشروع دون موافقة الإدارة.
   • يمنع إرسال روابط دعوات الديسكورد أو أي روابط ترويجية.

5. **الخصوصية**
   • يمنع نشر معلومات شخصية لك أو لغيرك.
   • يمنع مشاركة المحادثات الخاصة دون إذن أصحابها.

6. **الانتحال**
   • يمنع انتحال شخصية المشهور، الإدارة، أو أي عضو آخر.
   • يمنع استخدام أسماء أو صور قد تسبب تضليلاً للأعضاء.

7. **استخدام الرومات**
   • استخدم كل روم في الغرض المخصص له.
   • يمنع الخروج عن موضوع الروم بشكل متكرر.

8. **النقاشات**
   • النقاش مسموح باحترام.
   • يمنع إثارة الفتن أو الجدالات السياسية أو الدينية التي تسبب المشاكل.

9. **استغلال الثغرات**
   • يمنع استغلال أي خطأ أو ثغرة في السيرفر أو البوتات.
   • يجب إبلاغ الإدارة فور اكتشاف أي مشكلة.

10. **أوامر الإدارة**
    • يمنع الاعتراض على قرارات الإدارة داخل الرومات العامة.
    • في حال وجود مشكلة، افتح تذكرة وسيتم النظر فيها.

11. **الحسابات الوهمية**
    • يمنع استخدام الحسابات البديلة للتحايل على العقوبات أو إزعاج الأعضاء.

12. **العقوبات**
    • مخالفة القوانين قد تؤدي إلى: تنبيه، كتم، طرد، أو حظر دائم حسب نوع المخالفة.
    • يحق للإدارة اتخاذ الإجراء المناسب للحفاظ على نظام المجتمع.

✨ **ملاحظة:** دخولك للسيرفر يعني موافقتك على جميع القوانين المذكورة أعلاه. قد يتم تحديث القوانين في أي وقت، ويُعد استمرارك في السيرفر موافقة على أي تحديثات. نتمنى لك وقتاً ممتعاً، وشكراً لكونك جزءاً من مجتمعنا 💙"""
    
    try:
        await channel.send(rules_message)
        await interaction.response.send_message(f'✅ تم إرسال القوانين إلى {channel.mention}', ephemeral=True)
    except discord.Forbidden:
        await interaction.response.send_message('❌ لا أمتلك صلاحية الإرسال في هذه القناة.', ephemeral=True)
    except Exception as e:
        await interaction.response.send_message(f'❌ حدث خطأ: {e}', ephemeral=True)
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(
        name='📜 إرسال القوانين',
        value='إرسال رسالة القوانين إلى قناة محددة\nالأمر: `/rules`',
        inline=False
    )
